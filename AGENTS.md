...
- 前端消息展示直接消费官方 `BaseMessage[]`，只允许无状态的回合分组，不得定义自有 `Message`/`MessagePart`/`ToolCallInfo` 或复制消息内容。AI text/reasoning/tool call 统一读取 `AIMessage.contentBlocks`，不保留任何兼容回退入口。实时工具运行状态优先读取 `useToolCalls`，`liveToolCall.status` 决定 running/finished/error，收到 `finished/error` 即时更新对应工具 UI（`ToolMessage` 不再作为工具完成的必要条件，空结果同样视为成功）；业务展示采用两阶段数据源：优先消费 `liveToolCall.output` 实时展示结果，后续 `ToolMessage.artifact` 到达后作为最终持久化内容补充/覆盖，未定义业务呈现时显示“该步骤没有可展示的业务数据”，不回退为 loading；不得解析 ToolMessage content、匹配供应商错误字符串、推断“整理回答中”或自行计算思考耗时。
